...
A connected component (CC) is a maximal subgraph of an undirected graph in which there is a path from each vertex to any other vertex in the subgraph. We implemented and parallelized algorithms using PThreads to identify the number of connected components and the membership vector in large undirected graphs. Typical sequential approaches are BFS-based labeling and union-find.
